...
// Builds a human-readable label for the states a student is targeting.
// No single state is assumed as a default — "AI" means All-India quota,
// and an empty/unspecified list is just labeled generically.
function getStatesDisplay(states?: string[] | string): string {
  const list = Array.isArray(states) ? states : states ? [states] : [];
  if (list.length === 0) return 'Not specified';
  if (list.includes('AI')) return 'All India (AIQ)';
  return list.join(', ');
}

// Derives a short "quota" badge for the colleges section from whatever
// states actually appear in the selected colleges, instead of hardcoding
// one state's counselling body.
function getQuotaBadgeLabel(selectedColleges: any[]): string {
  const states = Array.from(
    new Set(
      (selectedColleges || [])
        .map((c: any) => c.state_name || c.state)
        .filter(Boolean)
    )
  );
  if (states.length === 0) return 'State / AIQ Quota';
  if (states.length === 1) return `${states[0]} State Quota`;
  return 'Multi-State Quota';
}
...
export function buildHTML(body: RequestBody): string {
  ...
  const statesDisplay = getStatesDisplay(studentProfile.states);
  const quotaBadgeLabel = getQuotaBadgeLabel(selectedColleges);

  const collegesHTML = (selectedColleges || []).length === 0
    ? '<div style="padding:20px;text-align:center;color:#64748b;font-weight:600;font-size:13px;">No colleges selected yet</div>'
    : (selectedColleges || []).map((c: any, i: number) => {
        const cName = c.college_name || c.name || `Medical College ${i + 1}`;
        const cState = c.state_name || c.state || 'Unspecified';
        const cCity = c.city_name || c.city || '';
        const cType = c.college_type || c.type || 'Government';
        const cutoff = c.closest_cutoff ? `AIR ~${Math.round(c.closest_cutoff).toLocaleString('en-IN')}` : 'TBA';
        const chance = c.best_chance || 'High';
        ...
        return `
          <div style="margin-bottom:20px;padding:16px;border:1px solid #cbd5e1;border-radius:12px;background:#ffffff;page-break-inside:avoid;">
            ...
            ` : `<div style="font-size:11px;color:#64748b;font-style:italic;">Counselling Schedule: Round 1 Choice Filling and Option Entry active on the relevant state/AIQ portal.</div>`}
          </div>
        `;
      }).join('');
  ...
  <div style="background:#0f172a;border-radius:12px;padding:20px;margin-bottom:20px;color:#ffffff;">
    <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#818cf8;margin-bottom:6px;">
      NEET AI Predictor &bull; Official Report
    </div>
    <h1 style="margin:0;font-size:20px;font-weight:800;">NEET Counselling Strategy &amp; Timeline Report</h1>
    <p style="margin:6px 0 0;font-size:12px;color:#cbd5e1;">
      Prepared for <strong style="color:#ffffff;">${name || 'Medical Student'}</strong> &bull; Rank: <strong style="color:#818cf8;">${studentProfile?.rank || 'N/A'}</strong> &bull; Generated ${generatedAt}
    </p>
  </div>
  ...
  <div style="border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:20px;background:#ffffff;">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;">
      <tr>
        <td style="width:33%;padding:6px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
          <div style="font-size:9px;font-weight:800;color:#64748b;text-transform:uppercase;">NEET AIR Rank</div>
          <div style="font-size:12px;font-weight:800;color:#4f46e5;margin-top:2px;">${studentProfile?.rank || 'N/A'}</div>
        </td>
        <td style="width:33%;padding:6px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
          <div style="font-size:9px;font-weight:800;color:#64748b;text-transform:uppercase;">Target Course</div>
          <div style="font-size:12px;font-weight:800;color:#0f172a;margin-top:2px;">${studentProfile?.course || 'MBBS'}</div>
  ...
  <div style="border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:20px;background:#ffffff;">
    <table style="width:100%;margin-bottom:14px;border-bottom:1px solid #f1f5f9;padding-bottom:10px;">
      <tr>
        ...
        <td style="text-align:right;">
          <span style="font-size:10px;font-weight:700;color:#4f46e5;background:#eef2ff;padding:3px 8px;border-radius:10px;">
            ${quotaBadgeLabel}
          </span>
        </td>
      </tr>
  ...
  <!-- Footer -->
  <div style="text-align:center;padding:12px;color:#94a3b8;font-size:10px;border-top:1px solid #e2e8f0;">
    <p style="margin:0;font-weight:700;color:#64748b;">NEET AI Predictor &bull; Official Medical Admissions Portal</p>
    <p style="margin:2px 0 0;">Report based on official state counselling authority and AIQ closing ranks and AI admission timelines.</p>
  </div>
</div>`;
}
